...
def get_real_market_news(ticker: str) -> str:
...
def run_financial_analyst(ticker, start, end, oai_config_path, api_keys_path, model_name):
    ...
    if not found_json:
        raise ValueError("Could not find valid report JSON in agent output.")
...
if __name__ == "__main__":
    ...
    # MODELS = ["claude-sonnet-4-20250514", "grok-4", "meta-llama/llama-4-maverick", "gpt-4o", "gpt-4o-mini", "gpt-5-mini", "gpt-5-nano", "gpt-5", "o3", "o4-mini" ]
    MODELS = ["grok-4"]
    CONFIG_PATH = r"D:\my-fin-project\OAI_CONFIG_LIST"
    KEYS_PATH = r"D:\my-fin-project\config_api_keys"
    OUT_DIR = r"D:\my-fin-project\academic_results"
    ...
    latex_cols = ["Model", "Status", "Latency (s)", "Est. Cost ($)", "Direction", "Output Length"]
    ...
    print(df[["Model", "Status", "Latency (s)", "Est. Cost ($)", "Direction"]])
