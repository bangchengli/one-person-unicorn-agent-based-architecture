# portfolio_benchmark/refdata.py
...
def _get_ticker_sector(ticker: str, sleep_time: float = 0.0) -> str:
    """
    Best-effort获取行业/领域描述。
    优先用 results.sector/industry，取不到时回退到 sic_description；再不行用 "Unknown"。
    """
...
def build_meta_df(tickers: List[str], start: str, end: str,
    ...
    """
    构建真实元数据表：columns = [ticker, sector, avg_dollar_vol]
    - per_req_sleep: 为了照顾限流，适当 sleep（秒）
    """
